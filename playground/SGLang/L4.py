"""L4: Diffusion inference fundamentals as a Python module."""
...
class Stage:
    ...
    def __init__(self, pipeline: DiffusionPipeline, device: str) -> None:
...
class Pipeline:
    ...
    def __init__(self, diffusers_pipeline: DiffusionPipeline, device: str) -> None:
...
class DiffusionInferenceLesson:
    """Class wrapper for the Lesson 4 notebook workflow."""
    ...
    def run_denoising_timelapse(
        ...
        with torch.inference_mode():
            ...
            lat = lats.to(vae.dtype)
            lat_s = (lat / vae.config.scaling_factor) + vae.config.shift_factor
            img = vae.decode(lat_s, return_dict=False)[0]
            snapshots.append(
                ("Start\n(noise)", image_processor.postprocess(img, output_type="pil")[0])
            )

            for i, t in enumerate(ts):
                timestep = t.expand(lats.shape[0])
                timestep = (1000 - timestep) / 1000
                latent_input = lats.to(denoising_stage.transformer.dtype).unsqueeze(2)
                ...
                lats = denoising_stage.scheduler.step(
                    noise_pred.to(torch.float32),
                    t,
                    lats,
                    return_dict=False,
                ...
                snapshots.append(
                    (
                        f"Step {i + 1}/{len(ts)}",
                        image_processor.postprocess(img, output_type="pil")[0],
                    )
                )
        ...
        fig.text(
            0.28,
            0.01,
            "Structure phase (big changes each step)",
            ha="center",
            fontsize=10,
            color="#D32F2F",
            fontweight="bold",
        ...
        fig.text(
            0.72,
            0.01,
            "Refinement phase (tiny changes - cacheable!)",
            ha="center",
            fontsize=10,
            color="#1565C0",
            fontweight="bold",
        ...
        plt.suptitle(
            "Stages 4-5: Denoising + Decoding - noise to pixels in 9 steps",
            fontsize=13,
            fontweight="bold",
            y=1.02,
        )
    ...
    def generate_full_pipeline(self) -> dict[str, Any]:
        ...
        output = self.engine.generate(self.base_params())
        return output
    ...
    def compare_cache_vs_no_cache(self, threshold: float = 0.30) -> tuple[BenchmarkResult, BenchmarkResult]:
        ...
        axes[0].text(
            0.5,
            -0.10,
            f"9/9 transformer calls  |  {res_no_cache.time_seconds:.2f}s",
            transform=axes[0].transAxes,
            ha="center",
            fontsize=10,
            color="#D32F2F",
        ...
        axes[1].text(
            0.5,
            -0.10,
            f"{res_cache.steps_executed}/9 calls ({res_cache.steps_skipped} skipped)"
...
def main() -> None:
    lesson = DiffusionInferenceLesson()
