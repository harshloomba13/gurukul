...
# Create a Snowflake session lazily
_snowpark_session = None

def get_snowpark_session():
    """Get or create the Snowflake session. Creates it only when needed."""
    ...
    if _snowpark_session is None:
        ...
        if not snowflake_connection_parameters.get("account"):
            raise ValueError(
                "SNOWFLAKE_ACCOUNT environment variable is not set. "
                "Please set Snowflake connection parameters in your .env file."
            )
...
# But don't create the session until it's actually accessed
class _LazySession:
    """Lazy wrapper for Snowflake session that only creates it when accessed."""
    ...
    def __repr__(self):
        if _snowpark_session is None:
            return "<LazySession (not yet created)>"
        return repr(_snowpark_session)

snowpark_session = _LazySession()
...
# Create cortex_agent_tool lazily - only when actually needed
_cortex_agent_tool = None

def get_cortex_agent_tool():
    """Get or create the CortexAgentTool. Creates it only when needed."""
    global _cortex_agent_tool
    if _cortex_agent_tool is None:
        _cortex_agent_tool = CortexAgentTool(session=get_snowpark_session())
    return _cortex_agent_tool

# For backward compatibility, create a lazy wrapper
class _LazyCortexAgentTool:
    """Lazy wrapper for CortexAgentTool that only creates it when accessed."""
    def __getattr__(self, name):
        tool = get_cortex_agent_tool()
    ...
    def __repr__(self):
        if _cortex_agent_tool is None:
            return "<LazyCortexAgentTool (not yet created)>"
        return repr(_cortex_agent_tool)

cortex_agent_tool = _LazyCortexAgentTool()
...
# Create cortex_agent lazily - only when actually needed
_cortex_agent = None

def get_cortex_agent():
    """Get or create the cortex_agent. Creates it only when needed."""
    global _cortex_agent
    if _cortex_agent is None:
        _cortex_agent = create_react_agent(llm, tools=[cortex_agent_tool.run], prompt=agent_system_prompt(f"""
    ...
    """))
    return _cortex_agent

# For backward compatibility, create a lazy wrapper
class _LazyCortexAgent:
    """Lazy wrapper for cortex_agent that only creates it when accessed."""
    def __getattr__(self, name):
    ...
    def __repr__(self):
        if _cortex_agent is None:
            return "<LazyCortexAgent (not yet created)>"
        return repr(_cortex_agent)

cortex_agent = _LazyCortexAgent()
