...
os.environ['OPENAI_MODEL_NAME'] = 'gpt-4-mini'
...
class BoardDataFetcherTool(BaseTool):
    ...
    def _run(self) -> dict:
        ...
        if not all([self.api_key, self.api_token, self.board_id]):
            return json.dumps({"error": "Missing Trello credentials. Please set TRELLO_API_KEY, TRELLO_API_TOKEN, and TRELLO_BOARD_ID environment variables."})
...
class CardDataFetcherTool(BaseTool):
  ...
  def _run(self, card_id: str) -> dict:
    if not all([self.api_key, self.api_token]):
        return json.dumps({"error": "Missing Trello credentials. Please set TRELLO_API_KEY and TRELLO_API_TOKEN environment variables."})
...
try:
    ...
    print("Note: Trello screenshot could not be displayed. This is expected when running outside of a Jupyter notebook or if the image file is missing.")
...
try:
    ...
    print("Note: Markdown display is not available. This is expected when running outside of a Jupyter notebook.")
