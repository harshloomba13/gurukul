...
class ACPCallingAgent(MultiStepAgent):
    ...
    async def step(self, memory_step: ActionStep) -> Union[None, Any]:
        ...
        for i, message in enumerate(memory_messages):
            if isinstance(message, dict) and "content" in message and not isinstance(message["content"], list):
        ...
        try:  
            # Convert dict messages to objects with role attribute for the model
            formatted_messages = []
            for msg in memory_messages:
                if isinstance(msg, dict):
                    # Create a simple object with role attribute
                    class MessageObj:
                    ...
                    formatted_messages.append(MessageObj(msg.get("role", "user"), content))
                else:
                    formatted_messages.append(msg)
            
            # Debug: Log the formatted messages
            self.logger.log(f"Formatted messages for model: {[(msg.role, msg.content) for msg in formatted_messages]}", level=LogLevel.DEBUG)
            ...
            self.logger.log(f"Model returned: content='{model_message.content}', tool_calls={model_message.tool_calls}, raw={model_message.raw}", level=LogLevel.DEBUG)
        ...
        if not hasattr(model_message, 'tool_calls') or model_message.tool_calls is None or len(model_message.tool_calls) == 0:
            ...
            if model_message.content and "final_answer" in model_message.content.lower():
                ...
                # If we have observations from previous steps, check if we should provide a final answer
                if any('_response' in key for key in self.state.keys()):
                    # We have some agent responses, let's see if we can provide a final answer
                    # Check if the user's query has been partially answered
                    user_query = "do i need rehabilitation after a shoulder reconstruction and what is the waiting period from my insurance?"
                    
                    # If we have health info but no insurance info, try to call policy_agent
                    if 'health_agent_response' in self.state and 'policy_agent_response' not in self.state:
                        # Try to call policy_agent for insurance information
                        self.logger.log(
                            "Have health response but missing insurance info, attempting to call policy_agent",
                            level=LogLevel.INFO,
                        )
                        # Create a synthetic tool call for policy_agent
                        memory_step.model_output = "Called Agent: 'policy_agent' with arguments: {'input': 'What is the waiting period for rehabilitation after shoulder reconstruction?'}"
                        memory_step.tool_calls = [ToolCall(name="policy_agent", arguments={"input": "What is the waiting period for rehabilitation after shoulder reconstruction?"}, id="synthetic_policy_id")]
                        return await self._process_tool_call(memory_step, "policy_agent", {"input": "What is the waiting period for rehabilitation after shoulder reconstruction?"})
                    else:
                        # We have enough information, provide final answer
                        responses = []
                        for key, value in self.state.items():
                            if '_response' in key:
                                responses.append(value)
                        
                        final_content = " ".join(responses)
                        if not final_content:
                            final_content = model_message.content or "I couldn't provide a complete answer."
                        
                        self.logger.log(
                            f"Providing final answer based on collected responses: {final_content}",
                            level=LogLevel.INFO,
                        )
                        memory_step.action_output = final_content
